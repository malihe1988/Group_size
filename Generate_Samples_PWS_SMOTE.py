...
# Function to retrieve parent and its nearest neighbors
def get_ngbr(df, knn, parent_idx):
    parent_candidate = df.iloc[parent_idx]
    if df.shape[0] < 2:
...
# Function to generate synthetic samples using PWS_SMOTE
def Generate_Samples_PWS_SMOTE(no_of_samples, parent_indices, df, df_name):
    """
    Generate synthetic samples using parent indices.

    Parameters:
        no_of_samples (int): Number of synthetic samples to generate.
        parent_indices (list): List of indices to use as parents for generating new samples.
    """
    total_data = df.values.tolist()
    num_samples = df.shape[0]
    n_neighbors = min(3, num_samples - 1)

    # Handle edge case where there are not enough neighbors
    if n_neighbors < 1:
        n_neighbors = 1
    ...
    for parent_idx in parent_indices:
        ...
        # Get the parent and its neighbors
        parent_candidate, child_candidate_1, child_candidate_2, child_candidate_3 = get_ngbr(df, knn, parent_idx)

        new_candidate = []

        # Iterate through each feature of the parent to generate synthetic values
        for key, value in parent_candidate.items():
            if isinstance(parent_candidate[key], bool):
                ...
                new_candidate.append(
                    random.choice([parent_candidate[key], 
                                    child_candidate_1[key], 
                                    child_candidate_2[key], 
                                    child_candidate_3[key]]))
        ...
        # Append the new candidate to the total data
        total_data.append(new_candidate)

    # Convert total data back to a DataFrame
